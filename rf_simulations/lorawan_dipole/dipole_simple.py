...
dipole_length = (0.46 * lambda_antenna_u)

# * Dimensions
dx_dip = dipole_length/200
'''
...
stop_dip1 = np.array([dx_dip/2, dy_dip/2, 0])
materialList['copper'].AddBox(start=start_dip1, stop=stop_dip1, priority=10)

## Feed
start_feed = np.array([-dx_feed/2, -dy_gap_dip/2, 0])
stop_feed = np.array([dx_feed/2, dy_gap_dip/2, 0])
...
idx = 0
mesh.x = np.concatenate((mesh.x, np.array([start_dip1[idx], start_feed[idx]])))
mesh.x = np.concatenate((mesh.x, np.array([stop_dip1[idx], stop_feed[idx]])))
idx = 1
mesh.y = np.concatenate((mesh.y, np.array([start_dip1[idx], start_feed[idx]])))
mesh.y = np.concatenate((mesh.y, np.array([stop_dip1[idx], stop_feed[idx]])))
idx = 2
mesh.z = np.concatenate((mesh.z, np.array([start_dip1[idx], start_feed[idx]])))
mesh.z = np.concatenate((mesh.z, np.array([stop_dip1[idx], stop_feed[idx]])))
...
# Direction of excitation -> Y
ports['portin1'] = FDTD.AddLumpedPort(port_nr=1, R=feed_R, start=start_feed, stop=stop_feed, p_dir='y', excite=1, priority=portin_prio)
...
if not len(idx)==1:
    ...
    xlabel('Theta (deg)')
    title('Frequency: {} MHz'.format(f_res/1e9))
    legend()
...
### Total Current
figure()
plot(ports[f'portin{1}'].i_data.ui_time[0], ports[f'portin{1}'].it_tot, 'r--', linewidth=2, label='it_in1')
grid()
legend()
ylabel('current [A]')
xlabel('Time (s)')
